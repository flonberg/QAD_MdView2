...
ini_set("error_log", "./log/getPlansByStartDateError.txt");
...
//ob_start(); var_dump($handle242);$data = ob_get_clean();fwrite($fp, "\r\n handle242  is  \r\n ". $data);  
...
function getFromNewData(){
  global $bodyObj, $fp, $handleBB, $handle242;
  if (is_array($bodyObj))
  $selStr = "SELECT * FROM QADWorkdaysSpec7 where StartDateDate >= '".$bodyObj[0]."' AND StartDateDate < '".$bodyObj[1]."'";
else
  $selStr = "SELECT * FROM QADWorkdaysSpec7 WHERE StartDateDate >= '".$bodyObj->startDate."' AND StartDate < '".$bodyObj->endDate."'";
  fwrite($fp, "\r\n newPplans selStr is \r\n $selStr");
